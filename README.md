...
Saya menambahkan potongan kode berikut pada file `main.html` di dalam `<script> </script>`
``` JavaScript
    ...
    if (productEntries.length === 0) {
      ...
      htmlString = `
        <div class="flex flex-col items-center justify-center min-h-[24rem] p-6">
          ...
          <p class="text-center text-gray-600 mt-4">No product data available in Skivy.</p>
        </div>`;
    } else {
        ...
        productEntries.forEach((item) => {
          ...
          const skin_type = DOMPurify.sanitize(item.fields.skin_type);

          htmlString += `
          <div class="relative break-inside-avoid bg-white shadow-lg rounded-lg border border-gray-200 transform hover:scale-105 transition-transform duration-300 flex flex-col">
            <!-- Product Info -->
            <div class="p-6 flex-grow">
              <h3 class="font-bold text-xl text-gray-800">${name}</h3>
              <p class="text-gray-600 text-sm mt-1">${description}</p>
              <div class="mt-3">
                <p class="text-lg font-semibold text-pink-600">RP${item.fields.price}</p>
              </div>
              <div class="mt-2">
                <p class="text-sm text-gray-500">Skin Type: ${skin_type}</p>
              </div>
            </div>

            <!-- Action Buttons (Edit, Delete) -->
            <div class="flex justify-end items-center p-4 border-t border-gray-200 space-x-2">
              <a href="/edit-product/${item.pk}" class="bg-yellow-500 hover:bg-yellow-600 text-white rounded-full p-2 transition duration-300 shadow-md flex items-center justify-center w-10 h-10">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                </svg>
              </a>
              <a href="/delete/${item.pk}" class="bg-red-500 hover:bg-red-600 text-white rounded-full p-2 transition duration-300 shadow-md flex items-center justify-center w-10 h-10">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                </svg>
              </a>
...
```
Kode tersebut berfungsi untuk menjadikan cards data product dapat mendukung AJAX GET

## Mengambil Data Product Menggunakan AJAX GET dan Memastikan Data yang Diambil Milik Pengguna yang *Logged-In*
Saya menambahkan potongan kode berikut pada file `views.py`
``` Python
...
```
Lalu menambahkan kode berikut pada file `main.html` di dalam `<script> </script>`
``` JavaScript
...
Menambahkan potongan kode berikut pada file `main.html` dan menyesuaikan tampilan tombol
``` HTML
...
Saya menambahkan fungsi `add_product_ajax` pada file `views.py`
``` Python
@csrf_exempt
@require_POST
...
Pada file `urls.py`, import fungsi yang baru dibuat tadi, lalu tambah path baru pada `urlpatterns`
```Python
path('create-product-entry-ajax', add_product_entry_ajax, name='add_product_entry_ajax')
```

## Menghubungkan Form yang Sudah di Buat Dalam Modal ke Path `/create-ajax/`
Pada potongan kode berikut, form dihubungkan dengan path `/create-ajax/`
```JavaScript
document.getElementById("submitProductEntry").addEventListener("click", async function(event) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error("Failed to add product entry. Please try again.");
      }
    } catch (error) {
      alert(error.message);
    }
...
Menambahkan kode fungsi `refreshProductEntries()` pada file `main.html` di dalam `<script> </script>`
```JavaScript
async function refreshProductEntries() {
    const productEntryCards = document.getElementById("product_entry_cards");
    productEntryCards.innerHTML = "";
